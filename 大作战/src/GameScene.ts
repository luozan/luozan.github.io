...
class GameScene extends egret.DisplayObjectContainer{
    ...
    private end = null;
    private lineToPoint = {x:0,y:0};
    private graphics = new egret.Shape();
    private monsterManager;
    public playerColor;
    public computerColor;
    public computerTotal = 0;
    public playerTotal = 0;
    public neutralColor;
    public levelID;
    public startTime = 0;
    private isFirst = false;
    public constructor(LevelID,isFirst:any){
        super();
    ...
    //创建基地
    private createRound(item,color,id,sprite,type):void{
        var shape:any = new egret.Shape();
        shape.anchorX = 0.5;
        shape.anchorY = 0.5;
        shape.x = item.x;
        shape.y = item.y;
    ...
    //事件
    private roundTouchBegin(event):void{
        if(event.target.playerBase.length <= 0){
            return;
        }
        this.begin = event.target;
    }
    private roundTouchEnd(event):void{
    ...
    private roundTouchEndOutSide():void{
        this.begin = null;
    ...
    //创建小圆圈
    public createSmallCircle(color,x,y,size):void{
        var shape:any = new egret.Shape();
        shape.x = x;
        shape.y = y;
    ...
    private isInCircle(x,y,r1,r2,x2,y2):boolean{
