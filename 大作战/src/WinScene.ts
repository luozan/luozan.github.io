 ...
 */
class WinScene extends egret.DisplayObjectContainer{

    private levelID;
    private storage = null;
    private time
    public constructor(levelID,time){
        super();
    ...
    private formatTime(time):string{
        var time:any = Math.floor(time/1000);
        var hour:any = Math.floor(time/3600);
        var min:any = Math.floor((time-3600*hour)/60);
        var s:any = Math.floor(time%60);
        if(hour < 10){
            hour = '0'+hour;
        }
        if(min < 10){
